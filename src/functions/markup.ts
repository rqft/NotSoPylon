...
export function codeblock(
  text: string,
  options: CodeblockFilterOptions = {}
): string {
...
export const escape = Object.freeze({
  all: (text: string, options: MarkupFilterOptions = {}): string => {
    ...
    text = text.replace(
      Regexes[Strings.ITALICS],
      Replacements[Strings.ITALICS]
    );
    text = text.replace(Regexes[Strings.BOLD], Replacements[Strings.BOLD]);
    text = text.replace(
      Regexes[Strings.CODESTRING],
      Replacements[Strings.CODESTRING]
    );
    text = text.replace(
      Regexes[Strings.SPOILER],
      Replacements[Strings.SPOILER]
    );
    text = text.replace(Regexes[Strings.STRIKE], Replacements[Strings.STRIKE]);
    text = text.replace(
      Regexes[Strings.UNDERLINE],
      Replacements[Strings.UNDERLINE]
    );
  ...
  codeblock: (text: string, options: CodeblockFilterOptions = {}): string => {
    const filter: CodeblockFilter = Object.assign(
      {},
      defaultCodeblockFilter,
      options
    );
  ...
  mentions: (
    text: string,
    replacement: string = Replacements.MENTION
  ): string => {
