...
export const DISCORD_TOKEN_EPOCH = 1293840000000;
...
export function generate(options: SnowflakeGenerateOptions = {}): Snowflake {
  ...
  const timestamp =
    (BigInt(options.timestamp as number) - epoch) % max.timestamp;
