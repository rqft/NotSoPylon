...
import { ActivityActionTypes, ApiVersion } from "./globals";
export enum Urls {
...
export const Assets = Tools.URIEncodeWrap({
  ...
  ICON: (hash: string, format: string = "png"): string => `/${hash}.${format}`,
});
...
export const Invite = Tools.URIEncodeWrap({
  LONG: (code: string): string => `${Urls.STABLE.slice(0, -1)}/invite/${code}`,
...
export const Routes = Tools.URIEncodeWrap({
  ...
  LOGIN_REDIRECT_TO: (redirectTo: string) => `/login?redirect_to=${redirectTo}`,
  ME: "/channels/@me",
  MESSAGE: (
    guildId: string | null,
    channelId: string,
