...
const defaultMainText = "/* hey theres no code here */";
...
async function getDeployment(gid) {
  if (
    typeof deploymentCache[gid] === "object" &&
    deploymentCache[gid] !== null
  ) {
...
async function getDeploymentIds() {
  ...
  if (isDebug === true) {
    ...
    if (dept) {
      const correctDep = dept.find(
        (vall) =>
          vall.disabled === false && vall.bot_id === "270148059269300224"
      );
  ...
  if (toadd.length > 0) {
    ...
    console.log(
      `${
        added.length > 0 ? `Added **${added.length}** guilds successfully!` : ""
      }${
        failToAdd.length > 0
  ...
  validGuilds = validGuilds
    .filter((val) => whitelist.includes(val.id))
    .map((v) => v.id);
  const notFound = whitelist.filter(
    (v) =>
      validGuilds.find((val) => val === v) === undefined &&
      !toRet.failed.includes(v)
  );
...
getDeploymentIds()
  .then(async (objDeps) => {
    ...
    if (typeof wh === "string" && ids.length > 1 && isGh && !isDebug) {
      ...
      if (skipped.length > 0) {
        txt += `\n🟡 Skipping **${skipped.length}** guild${
          skipped.length > 1 ? "s" : ""
        } ${skipped.map((e) => `\`${e}\``).join(", ")}`;
      }
      if (failed.length > 0) {
        txt += `\n🔴 Failed for **${failed.length}** guild${
          failed.length > 1 ? "s" : ""
        }: ${failed.map((e) => `\`${e}\``).join(", ")}`;
      }
      sendWebhook(txt);
    ...
    for (const k in ids) {
      ...
      try {
        ...
        if (typeof obj.msg === "string") {
          ...
          if (!isGh) {
            console.log(
              `Published to ${obj.guild.name}${
                isGh === false ? ` (${obj.guild.id}) ` : " "
              }successfully (Revision ${obj.revision})! `
            );
          } else {
          ...
          if (typeof wh === "string" && isGh && !isDebug) {
            if (ids.length >= lengthShorten) {
              toPost.push(
                `✅ \`${obj.guild.name}\` [||\`${obj.guild.id}\`||] (<@!${obj.bot_id}>) #${obj.revision}`
              );
            } else {
              toPost.push(
                `✅ \`${obj.guild.name}\` (<@!${obj.bot_id}>) - #${obj.revision}\nGID:[||\`${obj.guild.id}\`||]\nSID:[||\`${obj.script.id}\`||]\nDID:[||\`${deployment_id}\`||]`
              );
            }
          }
          if (isDebug && !isGh) {
        ...
        console.error(
          `Publish error: ${r.url} > ${r.status} - ${r.statusText}`
        );
        console.error(r);
      ...
      if (toPost.length === ids.length) {
        ...
        sendWebhook(
          toPost.join(`${ids.length >= lengthShorten ? "\n" : "\n\n"}`)
        );
      }
    }
  })
